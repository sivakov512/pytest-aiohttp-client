...
async def test_will_send_request_with_expected_method(method, api, req):
...
async def test_returns_payload_by_default(url, expected, api):
    got = await api.get(url)

    assert got == expected


async def test_returns_response_if_requested(api):
    got = await api.get("/url/", as_response=True)

    assert isinstance(got, ClientResponse)


async def test_asserts_for_status_code(api):
...
@pytest.mark.parametrize(
    "kw",
    [dict(data='{"k": "v"}'), dict(json={"k": "v"})],
)
async def test_passed_payload(kw, api, req):
